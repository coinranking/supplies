...
class Subscan extends Driver {
  constructor(options) {
    super({
      ...
      supports: {
        native: false,
        balances: true,
        blockchains: ['Polkadot', 'Kusama', 'Stafi'],
      },
      options,
    });
  }

  async totalSupplyStafi() {
    const { data: { detail } } = await this.request('https://stafi.subscan.io/api/scan/token');
    const { total_issuance: total, token_decimals: decimals } = detail.FIS;
    if (!total) {
      return undefined;
    }
  ...
  async balanceStafi(modifier) {
    const options = {
      url: 'https://stafi.subscan.io/api/open/account',
      method: 'post',
      header: {
