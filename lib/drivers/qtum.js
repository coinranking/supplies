...
class Qtum extends Driver {
  constructor(options) {
    super({
      ...
      supports: {
        ...
        balances: true,
        max: true,
        // tokens: true, this is currently not working because of fetchTokenTotalSupply
      },
      options: {
        ...options,
        useCache: true,
  ...
  /**
   * @note this api currently not working
   * @link https://github.com/qtumproject/insight-api#token-account-balance api
   * @link https://github.com/qtumproject/insight-api/issues/10 issue
   * @param reference
   */
  async fetchTokenTotalSupply(reference) {
    const supply = await this.request(
      `https://explorer.qtum.org/insight-api/tokens/${reference}/total-supply`,
    );
